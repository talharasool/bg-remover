...
class ImageProcessor:
    ...
    async def process_image(self, image_data: bytes) -> bytes:
        ...
        result: bytes = await loop.run_in_executor(None, self._remove_background, image_data)
        return result
    ...
    def _remove_background(self, image_data: bytes) -> bytes:
        ...
        output: bytes = remove(image_data, session=self.session)
        return output

    async def process_job_image(self, job_id: str, image_id: str, original_path: str, original_filename: str) -> str:
        """Process a single image from a job."""
        try:
            # Update status to processing
            self.job_manager.update_image_status(job_id, image_id, JobStatus.PROCESSING)
            ...
            processed_path = await self.storage.save_processed(processed_data, original_filename, job_id)

            # Generate download URL
            download_url = f"/api/v1/download/{job_id}/{image_id}"

            # Update status to completed
            self.job_manager.update_image_status(job_id, image_id, JobStatus.COMPLETED, download_url=download_url)

            return processed_path

        except Exception as e:
            # Update status to failed
            self.job_manager.update_image_status(job_id, image_id, JobStatus.FAILED, error=str(e))
            raise

    async def process_batch(self, job_id: str, images: list[dict]) -> None:
